...
2. Heat the agarose mixture in the microwave until the agarose is completely dissolved and the solution becomes clear. This should be accomplished in the following manner to avoid damaging the mixture. Heat the glass bottle in the microwave for ~30 secs at a time, swirling the mixture around in between heating sessions. **WATCH CAREFULLY while microwaving so it does not bubble over.**
...
Here, we will reheat the agarose solution which will be used to embed the larvae zebrafish later in the protocol. Doing this step this early allows for some time for the solution to cool down after reheating in the microwave so you do not cook your fish.

1. Select a single agarose test tube from the fridge. Place this tube in the microwave for 5 seconds at a time until the center of the agarose solution melts ie. turns clear. Once this happen, only heat the tube 2 to 3 seconds at a time, inspecting the tube between heating sessions. If this process is done too expediently, the top portion of the agarose will pop off in the microwave and you will lose half of your solution. 
2. Once the agarose has melted all the way through, you will want to maintain the agarose's temperature. To maintain agarose temperature: place the test tubes containing agarose in a 37–42 °C water bath, or onto a heating block. Leave the tube in this heat bath for a few minutes to allow the agarose to cool off. 
# Aligning the objective to the laser

### Principle Rules

1. Do not hit the objective against any other object as it is expensive and scratches on the objective will have negative implications on image quality. 
...
2. **Turn the laser on.** Flip laser switch at the back of the laser's housing into the 'on' position. You should see the "Power" light turn on. 
3. **Turn the camera on.** Flip the switch on the camera at the top of the scope.
4. **Turn on the computer and associate large monitor.** 
...
5. **Turn on the laptop.** 
	- This laptop controls the laser box. The laser housing contains 4 different wavelength lasers and this laptop will allow us to select which laser we would like to use as well as modulate the power with which it is lasing. 
	- The password to the laptop is '12zfish34'.
	- On the laptop, select the "Omicron Control Center" app.
	- Once the app opens, select the 488nm laser on the upper section.
...
8. **Enable the laser & camera**
	...
	- Select, "Scan Mode: Scanning Off". This will ensure that the galvanometers are off. In other words, the laser will simply be directed to a single point in the sample. **It will be our goal in this section to align the objective to this lasing point.**
	- Click the "OFF" button near the 488nm laser in the "Lasers" box in order to "Enable" or open the shutter for the 488nm laser to be emitted. You should not see the laser beam being emitted, hopefully, through some protective gogs.
	- Check that the laser line is unobstructed. In other words, make sure that the laser is able to reach the beam block on the other side of the sample housing.
...
9. **Align the objective to the laser**
	...
	- On the microscope, you will see two manual adjustment knobs, one for the X positioning and the other for the Y positioning. Adjust them until the laser line is roughly centered on the camera GUI. Follow the descending slope of the gaussian line to get to the center focus.
	- Now, to adjust the focus depth of the system ie. vary the distance between the objective and the sample, adjust the objective position using the main computer GUI. Turn to the 'Target tilt 1' square and adjust the Z position back and forth. A good starting point would be adjusting +/- 10 either way. 
	- Once the focus is roughly in the center of the screen, adjust the camera zoom  to -3 both in the X and Y directions. Adjust the Z position until the vertical width of the focus of the laser beam is ~15 pixels wide. 
	- Place your cursor to the center of the image being captured (1024, 1024). The location of the cursor will be displayed beneath the image being captured. In total, the image dimensions are 2048 by 2048 pixels. 
	- Using the X and Y knobs, bring the focus precisely to the center of the image being captured ie. where you placed your cursor. 
...
Here, we will embed the zebrafish larvae. The reason this step comes after the objective to laser alignment is because we want to prepare the sample as close to the imaging step as possible due to the larvae still being alive in the agarose and therefore, capable of movement.

At this point (from Part 1), a test tube with melted agarose solution should be found 
...
4. Pipette a larvae zebrafish. Release as much water as possible back into the larvae petri dish. Essentially, the goal is to have as little water in the pipette as possible.
...
6. Now, pipette the larvae zebrafish back up from the tube and release it into a **rectangular** ice-cream mold.
7. **Position the larvae zebrafish in the agarose.** Using the probe, finesse the larvae so that it is positioned as parallel to the sides of the mold as possible while minimizing the pitch and yaw angle. This is critical. When imaging, you will want the zebra fish's brain to be as orthogonal as possible to the objective above. Keep moving the fish around while the agarose has yet to solidify while also not touching the fish. This will require practice. It is also important to avoid moving the fish once the agarose has solidified. This will cause the agarose to form cracks and bubbles.
...
9. Cut the sides of the agarose block around the fish, being careful to avoid hurting the actual fish.
10. **Mount the prepared agarose-larvae block into the sample housing.** First, pipette a thin layer of agarose onto the beam in the sample housing for a layer of adhesion. Wait for this layer of agarose to solidify for a few seconds. Pick up the prepared agarose-larvae block and deposit it on top of the thin layer of agarose. The fish should be facing the direction of the mount which is not connected to the wall. Once placed, deposit a thin layer of agarose over the agarose-larvae block in order to make the sample sturdier.
